...
export function stripDiacritics(str) {
  const mapped = String(str).replace(/[øØđĐłŁßæÆœŒðÐþÞ]/g, (ch) => EXTRA_CHAR_MAP[ch]);
...
// Player names need special handling:
//  - API-Football commonly returns abbreviated first names ("D. Beljo")
//    while our database has full names ("Dominik Beljo") — so compare
//    surname and first-name/initial separately and weight surname more.
//  - Spanish/Portuguese names often carry two surnames (e.g. "Sergi
//    Domínguez Viloria"), but one source may only use the primary one
//    ("Domínguez"). Taking only the LAST token as "the surname" misses
//    that — so we check every token after the first name on both sides
//    and keep the best match, not just the last-vs-last comparison.
export function playerNameScore(nameA, nameB) {
  ...
  let lastSim = similarity(aLast, bLast);
  for (const tok of aTokens.slice(1)) lastSim = Math.max(lastSim, similarity(tok, bLast));
  for (const tok of bTokens.slice(1)) lastSim = Math.max(lastSim, similarity(aLast, tok));
