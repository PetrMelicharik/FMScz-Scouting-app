...
// Pure — takes an already-filtered comparison pool (rows) and the dataset's
// column list, so it works equally from server code (via loadPlayersData,
// see lib/pizzaData.js) or from a client component that fetched
// /data/players.json itself (e.g. the player-comparison page).
export function computePizzaFromPool(player, pool, cols, group) {
  const idx = {};
